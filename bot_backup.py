...
async def cmd_random_video(message: types.Message):
    ...
    if video:
        ...
        text = get_text(lang, "random_video").format(
            title=video_title, duration=duration, description=description, url=video_url
        )
...
async def setup_language(message: types.Message, state: FSMContext):
    ...
    if lang_code == "ru":
        await message.answer(
            "💰 Теперь выберите валюту:", reply_markup=get_currency_keyboard()
        )
    elif lang_code == "kz":
        await message.answer(
            "💰 Енді валютаны таңдаңыз:", reply_markup=get_currency_keyboard()
        )
    elif lang_code == "ua":
        await message.answer(
            "💰 Тепер виберіть валюту:", reply_markup=get_currency_keyboard()
        )
    else:
        await message.answer(
...
async def transaction_category(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        get_text(lang, "enter_description"), reply_markup=get_cancel_keyboard(lang)
    )
...
async def transaction_note(message: types.Message, state: FSMContext):
    ...
    db.add_transaction(
        message.from_user.id, data["type"], data["amount"], data["category"], note
    )
    ...
    await message.answer(
        get_text(lang, "transaction_saved").format(
            type=(
                get_text(lang, "income_text")
                if data["type"] == "income"
                else get_text(lang, "expense_text")
            ),
            amount=data["amount"],
...
@dp.message(
    lambda m: m.text
    in [
        get_text("ru", "statistics"),
        get_text("kz", "statistics"),
        get_text("en", "statistics"),
        get_text("ua", "statistics"),
    ]
)
async def show_statistics(message: types.Message):
    ...
    text += (
        get_text(lang, "stats_income").format(
            amount=income, currency=CURRENCIES[currency]["symbol"]
        )
        + "\n"
    )
    text += (
        get_text(lang, "stats_expense").format(
            amount=expense, currency=CURRENCIES[currency]["symbol"]
        )
        + "\n"
    )
    text += "━━━━━━━━━━━━━━━━\n"
    text += (
        get_text(lang, "stats_balance").format(
            amount=balance, currency=CURRENCIES[currency]["symbol"]
        )
        + "\n\n"
    )
...
async def show_goals(message: types.Message, state: FSMContext):
    ...
    for goal_id, name, target, current in goals:
        ...
        plant_display = get_plant_text(
            plant_type, percent, current, target, CURRENCIES[currency]["symbol"], lang
        )
    ...
    await message.answer(
        text, parse_mode="HTML", reply_markup=get_goal_actions_keyboard(lang)
    )
...
@dp.message(
    lambda m: m.text
    in [
        get_text("ru", "delete_goal"),
        get_text("kz", "delete_goal"),
        get_text("en", "delete_goal"),
        get_text("ua", "delete_goal"),
    ]
)
...
@dp.message(
    lambda m: m.text
    in [
        get_text("ru", "settings"),
        get_text("kz", "settings"),
        get_text("en", "settings"),
        get_text("ua", "settings"),
    ]
)
...
@dp.message(
    lambda m: m.text
    in [
        get_text("ru", "delete_all_data"),
        get_text("kz", "delete_all_data"),
        get_text("en", "delete_all_data"),
        get_text("ua", "delete_all_data"),
    ]
)
...
async def delete_all_data_execute(message: types.Message, state: FSMContext):
    ...
    if message.text == get_text(lang, "confirm_yes"):
        ...
        await message.answer(
            get_text(lang, "all_data_deleted"), reply_markup=get_main_keyboard(lang)
        )
    elif message.text == get_text(lang, "confirm_no"):
        await message.answer(
            get_text(lang, "deletion_cancelled"), reply_markup=get_main_keyboard(lang)
...
@dp.message(
    ...
    in [
        ...
        get_text("kz", "change_language"),
        get_text("en", "change_language"),
        get_text("ua", "change_language"),
    ]
)
...
@dp.message(
    lambda m: m.text
    in [
        get_text("ru", "change_currency"),
        get_text("kz", "change_currency"),
        get_text("en", "change_currency"),
        get_text("ua", "change_currency"),
    ]
)
...
async def join_shared_goal_execute(message: types.Message, state: FSMContext):
    ...
    if result is None:
        ...
        await message.answer(
            "⚠️ You are already a member of this goal!",
            reply_markup=get_shared_goals_keyboard(),
        )
    else:
...
async def process_shared_goal_amount(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        completed = db.add_to_shared_goal(
...
@dp.message(
    ...
    in [
        ...
        get_text("kz", "main_menu"),
        get_text("en", "main_menu"),
        get_text("ua", "main_menu"),
    ]
)
...
async def add_money_to_goal_execute(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        db.cursor.execute(
            "UPDATE goals SET current = ? WHERE id = ?", (new_current, goal_id)
        )
        db.conn.commit()
...
async def handle_unknown(message: types.Message, state: FSMContext):
    ...
    if user:
        lang = user[3]
        await message.answer(
            get_text(lang, "error"), reply_markup=get_main_keyboard(lang)
        )
    else:
...
async def main():
    ...
    await bot.set_my_commands(
        [
            ...
            types.BotCommand(command="new_goal",
                             description="🎯 Создать новую цель"),
            types.BotCommand(
                command="ask", description="🤖 Спросить у ИИ ассистента"),
            types.BotCommand(command="premium",
                             description="💎 Премиум доступ"),
            types.BotCommand(
                command="tip", description="💡 Получить финансовый совет"),
            types.BotCommand(command="video", description="📺 Случайное видео"),
            types.BotCommand(
                command="export_csv", description="📁 Экспорт данных в CSV"
            ),
            types.BotCommand(
                command="discount", description="🪙 Обменять монеты на скидку"
            ),
        ]
    )
    ...
    print(
        "✅ Countries: USA, Kazakhstan, Russia, Ukraine, Belarus, Uzbekistan, Kyrgyzstan"
    )
    print(
        "✅ Features: Shared Goals, Videos, Tips, Export, Goal Flowers 🌸, AI Assistant 🤖, Premium 💎"
    )
    print("=" * 50)
